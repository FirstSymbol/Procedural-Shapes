...
{
    [CustomEditor(typeof(ProceduralSprite))]
    [CanEditMultipleObjects]
    public class ProceduralSpriteEditor : UnityEditor.Editor
    {
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            VisualElement firstSection = root.Q<VisualElement>(className: "ps-section-content");
            firstSection.Insert(0, new PropertyField(m_Size, "Size (World Units)"));
            firstSection.Insert(1, new PropertyField(m_Color, "Tint Color"));
            firstSection.Insert(2, new PropertyField(serializedObject.FindProperty("m_AutoGenerateCollider"), "Auto Generate Collider"));
            ...
            // Remove or repurpose Bake collider button
            var bakeBtn = root.Q<Button>("bake-collider-button");
            if (bakeBtn != null) bakeBtn.style.display = DisplayStyle.None; // We use auto generate

            var raycastContainer = root.Q<VisualElement>("raycast-controls");
            if (raycastContainer != null) raycastContainer.style.display = DisplayStyle.None;
                ...
                case ShapeType.Path:
                    ...
                    pathField.RegisterValueChangeCallback(evt => {
                        foreach (var targetObj in targets) {
                            var shape = targetObj as ProceduralSprite;
        ...
        private void RefreshEffectsList()
        {
            ...
            for (int i = 0; i < m_Effects.arraySize; i++)
            {
                ...
                enabledToggle.RegisterValueChangedCallback(evt => { 
                    ...
                    serializedObject.ApplyModifiedProperties(); 
                    ((ProceduralSprite)target).SetAllDirty();
                });
        ...
        {
            ProceduralSprite shape = (ProceduralSprite)target;
        ...
        private void OnSceneGUI()
        {
            ...
            Matrix4x4 geomMatrix = shape.transform.localToWorldMatrix * Matrix4x4.Translate((Vector3)pivotOffset);
            ...
            using (new Handles.DrawingScope(geomMatrix))
            {
                if (shape.m_ShapeType == ShapeType.Rectangle)
                    ...
                    DrawLineHandles(shape, geomMatrix);
        ...
        private void DrawRectangleHandles(ProceduralSprite shape, float hw, float hh)
        {
        ...
        private float ModernHandle(Vector3 corner, Vector3 dir, float radius, string label, float hw, float hh, Color color)
        {
            ...
            if (newRadius > 0.1f) {
                ...
                Handles.Label(pos + dir * 0.5f, $"{label}: {newRadius:F2}", style);
            }
            
            return newRadius;
        }

        private void DrawLineHandles(ProceduralSprite shape, Matrix4x4 geomMatrix)
        {
            Matrix4x4 rtToGeom = geomMatrix.inverse * shape.transform.localToWorldMatrix;
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                shape.m_LineStart = shape.transform.worldToLocalMatrix.MultiplyPoint3x4(geomMatrix.MultiplyPoint3x4(start));
                shape.m_LineEnd = shape.transform.worldToLocalMatrix.MultiplyPoint3x4(geomMatrix.MultiplyPoint3x4(end));
                shape.SetAllDirty();
            }
        }

        private void DrawStarHandles(ProceduralSprite shape, Vector2 size)
        {
        ...
        private void DrawRingHandles(ProceduralSprite shape, Vector2 size)
        {
        ...
        private void DrawPathHandles(ProceduralSprite shape, Matrix4x4 geomMatrix)
        {
        ...
        private void DrawControlPoint(ref Vector2 cp, Vector2 anchor, ProceduralSprite shape, ShapePath path, int idx, Color color)
        {
        ...
        private void DrawGradientHandles(ProceduralSprite shape, float hw, float hh)
        {
